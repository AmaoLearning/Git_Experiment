...
{
    //Fill in your code here
    if (trow >= 0 || trow < units.numRows()) {
        if (tcol >= 0 || tcol < units.numCols()) {
            if (units[trow][tcol] == nullptr && terrains[trow][tcol] == PLAIN) {
                units[trow][tcol] = units[srow][scol]; // only change the pointer while not changing what it's pointing to
                delete units[srow][scol];
            }
        }
    }
    return true;
